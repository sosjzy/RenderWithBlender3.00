...
def setup_environment(params_rendering):
    ...
    scene.render.tile_x = int(params_rendering['tile_x'])
    scene.render.tile_y = int(params_rendering['tile_y'])
    ...
    bpy.context.scene.view_layers['View Layer'].cycles.use_denoising = str2bool(
        params_rendering['denoising_legacy'])
    bpy.context.scene.view_layers['View Layer'].cycles.use_sky = False
    bpy.context.scene.view_layers['View Layer'].cycles.use_ao = False
    bpy.context.scene.view_layers['View Layer'].cycles.use_strand = False
...
def set_material(idx, params_current_shader, current_object, working_dir):
    ...
    # Lambertian fix
    if pbs_node.inputs[7].default_value == 0.:
        print("run Lambertian reflectance")
        bsdf_node = mat.node_tree.nodes['Principled BSDF']
        pbs_node = mat.node_tree.nodes.new('ShaderNodeBsdfDiffuse')
        # print(params_current_shader['roughness'])
        pbs_node.inputs[1].default_value = 0  # params_current_shader['roughness']
        pbs_node.inputs[0].default_value = bsdf_node.inputs[0].default_value
        isLambert = True
    ...
    if bpy.data.objects[current_object].active_material is None:
...
def write_info(out_dir_v, filenames, light_type, lights, obj_names):
    ...
    with open(os.path.join(out_dir_v, 'filenames.ini'), 'w') as f:
        ...
        if light_type == 'POINT':
            f.write("\n\n[light locations]")
        if light_type == 'AREA':
            f.write("\n\n[light rotations and locations]")

        for i in range(0, len(filenames)):
            f.write("\n{:03d} = ".format(i + 1))
            for j in lights[i]:
                if light_type == 'SUN' or light_type == 'POINT':
                    f.write("{0:.5f}, {1:.5f}, {2:.5f}\t".format(j[0], j[1], j[2]))
                elif light_type == 'AREA':
                    f.write("{0:.5f}, {1:.5f}, {2:.5f}, {3:.5f}, {4:.5f}, {5:.5f}\t".format(j[0], j[1], j[2], j[3], j[4], j[5]))
        f.write("\n\n[info]")
...
def create_normal_map(filepath, pose_id=0):
    '''
...
def create_depth_map(filepath, pose_id=0):
    '''
...
def render_object(object_shape, params_rendering, params_camera, params_object_global,
    ...
    for cidx, pose in enumerate(camera_poses_set):
        ...
        create_normal_map(filepath=out_dir_v, pose_id=cidx)
        create_depth_map(filepath=out_dir_v, pose_id=cidx)
        if 'use_dof' in params_camera and str2bool(params_camera['use_dof']):
