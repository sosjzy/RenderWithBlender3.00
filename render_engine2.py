...
def set_material(idx, params_current_shader, current_object, working_dir):
    ...
    for key, value in params_current_shader.items():
        if key.lower().replace('_', ' ') in ["subsurface", "metallic", "specular", "specular tint", "roughness", "anisotropic",
                                             ...
                                             "transmission", "transmission roughness", "alpha","normal"] and value != '':
    ...
    for key, value in params_current_shader.items():
        ...
        if key.lower() in ["color_texture", "metallic_texture", "specular_texture", "roughness_texture", "sheen_texture",
        ...
        elif key.lower() in ["normal_texture"]:
            ...
            normalmap_node.uv_map = "UVMap"
            mat.node_tree.links.new(normalmap_node.outputs[0], pbs_node.inputs[22])
            mat.node_tree.links.new(texture_node.outputs[0], normalmap_node.inputs[1])

            print(pbs_node.inputs[22])
            print("成功创建normal map节点")
            print(mat.node_tree)
...
def set_texture_mapping(idx, key, value, working_dir, uv_flag):
    ...
    if key.lower() == "normal_texture": 
        print("已经找到normal贴图，路径："+str(value))
        socket = 20
    if uv_flag:
